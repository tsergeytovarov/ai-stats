...
final class ConfigTests: XCTestCase {
    func test_decodes_full_config() throws {
        ...
        XCTAssertEqual(cfg.ccusageCommand, ["bunx", "-y", "ccusage@latest"])
...
struct Config: Equatable {
    ...
    static func decode(from data: Data) throws -> Config {
        ...
        return Config(
            ...
            ccusageCommand: container.ccusageCommand ?? ["bunx", "-y", "ccusage@latest"],
...
struct CcusageFetcher: Fetcher {
    let commandPrefix: [String]    // e.g. ["bunx", "-y", "ccusage@latest"]
    ...
    func fetch(since: Date) async throws -> FetchResult {
        ...
        let head = commandPrefix.first ?? "bunx"
...
и ~/.bun/bin — иначе .app не находит bunx из GUI-окружения."
```
...
- Xcode 15+ для сборки
- [bun](https://bun.sh/) или Node.js (для `bunx ccusage`)
...
## Подготовка
- [ ] `bun --version` печатает версию (>= 1.0)
