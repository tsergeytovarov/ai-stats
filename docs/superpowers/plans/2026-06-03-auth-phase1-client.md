...
Обновить фабрику VM аккаунта (плюс хук live-пересборки источников после входа):
...
```swift
    func makeAccountTabViewModel() -> AccountTabViewModel {
        ...
        // После GitHub-входа GitHub-источника могло не быть в configuredSources
        // (на старте токена не было). Пересобираем источники и перезапускаем таймер,
        // чтобы коммиты/LOC заработали без перезапуска app.
    ...
    /// после OAuth) и перезапускает периодический sync. GitHub-источник дёргаем
    /// один раз сразу — чтобы данные появились немедленно.
...
> aiuse-синк (snapshot push) подхватывает новый device_token автоматически: `AiuseAPIClient.secretProvider` читает `secretBox.value`, который обновляется при входе. Пересобирать нужно только GitHub-источник — его fetcher строится с токеном/логином, зашитыми на момент `buildFetchers()`.
...
```swift
    /// Колбэк, дёргается после успешного GitHub-входа. AppContainer вешает сюда
    /// live-пересборку sync-источников (чтобы GitHub-стата заработала без перезапуска).
    /// nil в тестах → no-op.
...
```swift
    ...
    func signInWithGitHub(includePrivate: Bool) async {
        ...
        do {
            ...
            state = .created(row)
            await onSignedIn?()   // live-пересборка sync-источников (GitHub без перезапуска)
...
**Сознательно НЕ в этом плане:** link-intent / привязка существующего api_secret-аккаунта (пользователь выбрал «пересоздать фреш»); хранение `global_opt_in`/`sharing` состояния тогла локально между запусками (push-only в Phase 1, тогл оптимистичный); стрики/бейджи (фаза 2+).

> Live-пересборка GitHub-источника после входа **сделана** (Task 7 `refreshSourcesAfterSignIn` + хук `onSignedIn` в VM) — GitHub-стата активируется без перезапуска. aiuse-синк подхватывает device_token автоматически через `secretBox`.
...
- **`MyProfileRow` поля** (Task 8 Step 3) сверены с `Shared/Storage/Models.swift:156` — совпадают (id, friendCode, displayName, avatarPath, sharingEnabled, serverUserId, avatarBlob, avatarMime, avatarEtag), все с дефолтами где нужно. Инициализатор валиден.
