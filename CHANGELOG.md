...
- **opencode считается наравне с Claude Code и Codex.** Оказалось, ccusage
  умеет его с двадцатой мажорной — той самой, что у нас в дефолте, и отдаёт
  ровно ту же схему, что и Claude. Отдельный парсер не понадобился: opencode
  добавлен в список провайдеров, дальше траты сами едут в общий счёт, топ
  моделей, спарклайн и виджеты. Цена приходит от ccusage — наш прайс-лист про
  glm и прочих провайдеров opencode ничего не знает.
- **У тех, кто уже поставил Burn, провайдер включится сам.** Список провайдеров
  лежит в `~/.config/ai-stats/config.json` и сам бы не обновился. Разовая
  миграция дописывает opencode и сбрасывает окно синка, чтобы подтянулась вся
  история, а не последняя неделя. Если ты убрал провайдера из списка руками —
  назад мы его не вернём.

## [1.1.4] — 2026-07-31

### Fix: кольца лимитов плохо читались в светлой теме

- **Трек и контур кольца подстраиваются под тему меню-бара.** На светлом фоне
  белое кольцо OpenCode почти растворялось: трек был светлым в обеих темах, а
  контура в полпункта не хватало. Теперь основа трека меняется по теме, а
  контур на светлом вдвое плотнее и чуть толще. Цвета самих провайдеров
  остались постоянными — подкручено только обрамление.

## [1.1.3] — 2026-07-31

### Fix: полоски лимитов не выстраивались в колонку

- **Ширина полоски зафиксирована.** Полоска была единственным резиновым
  элементом строки и забирала место, оставшееся после хвоста со временем
  сброса, — а хвост у каждой строки своей длины («сброс через 4д 12ч» против
  «сброс через 27д 5ч»). Полоски начинались и кончались на разных местах,
  колонки не получалось. Теперь резиновый хвост, полоска фиксированная.
- **Заливка не может вылезти за дорожку.** Процент клампится: если источник
  когда-нибудь пришлёт перерасход квоты, заливка не поедет поверх соседней
  колонки.

## [1.1.2] — 2026-07-31

### Правки по первому живому взгляду на лимиты

- **Попап прокручивается.** Секция лимитов добавила контента, и он перерос
  высоту окна: строка последнего синка и кнопки обновления, настроек и выхода
  уезжали за границу, добраться до них было нельзя. Прокручивается контент,
  футер и остров вкладок остаются на месте.
- **Кольца в капсуле больше не слипаются.** Обводка центрируется на контуре
  круга и половиной толщины выходит за номинальный диаметр — прежний зазор в
  3pt она съедала целиком, и три кольца читались как одно пятно.
- **Цвет провайдера постоянный.** Codex синий, Claude рыжий, OpenCode белый —
  всегда, в кольце и в полоске. Пороги, на которых цвет уходил в жёлтый и
  красный, убраны: три кольца идут подряд без подписей, и цвет — единственное,
  чем они отличаются. Покрасневший на 96% Codex превращал ряд в три одинаковых
  пятна. Насколько всё плохо, показывает заполненность.

## [1.1.1] — 2026-07-31

### Fix: лимиты Claude и OpenCode разбирались неверно на живых данных

- **Время сброса разбирается с дробными секундами.** Живой эндпоинт отдаёт его
  в виде `2026-07-31T18:30:00.602320+00:00`, а разбор шёл по формату без дробной
  части и молча отдавал пустоту. Итог: строка «сброс через N» у Claude не
  показывалась ни разу, а выбор окна для кольца уходил в запасную ветку. Codex и
  OpenCode это не задевало — они отдают время сброса числом.
- **Лимиты OpenCode находятся на реальной странице.** Между именем поля и его
  значением страница вставляет обратную ссылку вида `$R[33]=`, а разбор ждал
  фигурную скобку сразу после двоеточия. При полностью исправной cookie лимиты
  OpenCode не показывались вообще — статус был «не разобрал страницу».

## [1.1.0] — 2026-07-31

### Мониторинг лимитов: сколько осталось у Claude, Codex и OpenCode

- **Три кольца в капсуле меню-бара** справа от цены — по кольцу на подписку.
  Кольцо заполняется по окну, которое сбрасывается ближайшим, а красится по
  худшему окну: расклад «пятичасовое на нуле, недельное на 95%» обязан выглядеть
  тревожно, а не спокойно.
- **Разбор по окнам на вкладке «Расходы»** — процент и время до сброса по
  каждому окну каждой подписки, с подписью, на какой момент сняты данные.
- **Три разных источника.** Codex отдаёт лимиты локально, через свой app-server;
  при недоступности берём последний снимок из rollout-логов и честно помечаем
  данные несвежими. Claude — через oauth-эндпоинт, не чаще раза в час: он
  троттлится и отвечает 429 с почти часовым Retry-After. OpenCode — скрейпом
  страницы подписки по сессионной cookie, которую нужно один раз вставить в
  настройках.
- **Ничего не выдумывается.** Нет данных — плашка, а не ноль. Протух токен или
  cookie — строка с тем, что надо сделать, а не молча показанные старые цифры.
- **История лимитов пишется в базу ступенями**, только при изменении. Это
  фундамент под следующий шаг — прогноз «при текущем темпе Codex кончится
  завтра к 18:00».

## [1.0.2] — 2026-07-31

### Fix: синк расходов вставал намертво и молча — цифры замирали на нуле

- **Вывод ccusage читается во временный файл, а не в пайп.** Пайп держит 16 КБ; когда ccusage писал больше, он навсегда блокировался в `write()`, а Burn — в ожидании его выхода, потому что читал пайп уже после завершения процесса. Зависший процесс жил сутками, источник `ccusage` оставался помеченным как «синхронизируется», и обе ветки — Claude и Codex — переставали обновляться. Ошибки в логе при этом не было: трата за день просто застывала, а новый день открывался нулём.
- **Таймаут 120 секунд на запуск ccusage.** Если процесс не ответил — SIGTERM, при необходимости SIGKILL, ошибка в `sync_state.last_error`. Синк идёт дальше вместо того, чтобы залипнуть до перезапуска приложения.
- **Пустой stdin для дочернего процесса** — npx больше не может подвиснуть на запросе подтверждения.
