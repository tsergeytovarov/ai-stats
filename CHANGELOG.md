...
### Fix: токены в «Аналитике» считались с кэшем — расходились с «Расходами» на два порядка

- **Топ моделей по токенам и сводки по источникам теперь считаются без кэша** (input+output), той же методикой, что вкладка «Расходы». Раньше карточка суммировала ещё и `cache_read`/`cache_creation` — на cache-heavy сессиях Claude Code это раздувало цифру в сотни раз (4.27 млрд токенов у Opus в «Аналитике» против ~614 млн за месяц на дашборде для всех моделей разом). Изначальное решение включать кэш опиралось на предположение, что карточка меряет нагрузку на лимиты Codex — но эта нагрузка считается отдельно, напрямую из ответа Codex API, и от суммы токенов не зависит.

## [1.0.0] — 2026-07-11

Крупный редизайн: приложение сфокусировано на тратах и на том, чем эти траты можно урезать. Социальный слой убран, вместо него — локальный советник по выбору модели.

### Added: вкладка «Аналитика» — советник по моделям за 30 дней

- **Новая вкладка в popover'е рядом с «Расходами».** Burn читает историю Claude Code и Codex локально и показывает, где дорогая модель тратилась на задачу, которую потянула бы модель дешевле. За фиксированное окно 30 дней, без переключателя периода.
- **Оценка утечки:** сколько денег уходит на переусложнённых моделях и какую долю недельного/месячного лимита это съедает.
- **Главные утечки и что делать:** регулярные задачи и роли пайплайнов на дорогой модели сворачиваются в кластеры с конкретным советом, чем заменить. Подтверждения («да», «ок», «1») и фоновые уведомления в утечки не попадают.
- **Топ-6 моделей за 30 дней по числу токенов** (не по деньгам) — горизонтальные бары.
- **Шпаргалка «какую модель когда»:** Codex — из live-описаний, Claude — статический справочник.
- **Строка советника в Large-виджете** на месте бывшего лидерборда.
- Разбор идёт полностью на устройстве. Обрезанные до 300 символов тексты промптов лежат только в локальной БД и на сервер не уходят.

### Removed: GitHub-статистика и социальный слой

- **Убраны вкладки «GitHub» и «Друзья».** Popover теперь — две вкладки: «Расходы» и «Аналитика». Лидерборд, снапшоты для сервера, кэши профилей друзей и вся обвязка шаринга статистики удалены.
- **Вход через GitHub остаётся** (Settings → «Аккаунт») — только для авторизации, не для соцфич.

### Changed: прайсы моделей 2026

- **Добавлены ставки** gpt-5.6-sol / gpt-5.6-terra / gpt-5.6-luna и gpt-5.3-codex-spark.
- **Family-fallback для gpt-5.6-*** — неизвестная модель семейства считается по ставке семейства, а не проваливается в дефолт.

### Changed: ингест истории

- **История агентов подбирается раз в час** в общем refresh-цикле — карточка аналитики не ждёт перезапуска.

## [0.6.8] — 2026-06-14

### Fix: токены показываются без кэша — одинаково на дашборде и борде

- **Число токенов везде (AI-вкладка, топ моделей, лидерборд) считается без cache reads.** Раньше главный экран показывал input с кэшем — на cache-heavy сессиях это раздувало цифру в десятки раз (например 530M вместо ~19M реального «свежего» объёма) и не совпадало с лидербордом. Теперь и дашборд, и борда показывают `input_tokens_no_cache` — одну и ту же честную цифру. Стоимость (`$`) при этом не меняется: она считается по фактическим ставкам всех токенов, включая кэш.

## [0.6.7] — 2026-06-14

### Fix: кнопки переключателей кликались только по тексту

- **Сегменты категории (AI / GitHub / Друзья) и периода (День / Неделя / Месяц) теперь кликабельны всей областью**, а не только по самому тексту. Фон неактивного сегмента прозрачный, а SwiftUI не считает прозрачное зоной тапа — добавлен `contentShape`, и padding вокруг текста больше не мёртвая зона.

### Fix: в настройках всегда показывалась версия 0.2.0

- **Футер настроек показывает реальную версию приложения** (из `Bundle`), а не захардкоженную «0.2.0», висевшую там с первых сборок. Путало: выглядело, будто установлена древняя версия, хотя сборка новая.

### Fix: статистика не доезжала до лидерборда после включения шаринга

- **При включении «Шарить статистику» снапшоты выгружаются сразу и заливают всю накопленную историю, а не только последние 7 дней.** Раньше тумблер лишь переключал серверный флаг: выгрузка ждала периодического тика (до 15 минут) и слала максимум неделю — кто включал шаринг не в первый день, светил на борде нулём. Вдобавок при рассинхроне локального флага с серверным (профиль создан до включения шаринга) клиент молча не слал вообще ничего. Теперь локальный флаг сверяется с сервером при входе и открытии настроек, история догоняется батчами, и каждый исход синка пишется в лог.
- **Снапшоты больше не «хоронятся» навсегда после серии сетевых сбоев.** Транзиентная ошибка отправки (например `-1005 network connection was lost`) растила счётчик попыток, и после 5 неудач снапшот выпадал из очереди безвозвратно — даже когда связь восстанавливалась. Теперь backfill сбрасывает счётчик, возвращая застрявшие снапшоты в отправку.

## [0.6.6] — 2026-06-14

### Fix: friend-код копировался без дефисов

- **Кнопка «Копировать» в Settings → «Аккаунт» теперь кладёт код с дефисами** (`XK7P-3M9Q-2A`), как он и показан на экране. Раньше в буфер уходила сырая форма без дефисов, и дефисы приходилось расставлять руками. Форматирование заодно вынесено в общий `FriendCode.formatted` (была копипаста в двух вьюхах) и покрыто тестами.
