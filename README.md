...
> **Статус:** personal MVP. Внутренние идентификаторы (bundle ID `com.sergeytovarov.aistats`, пути `~/.config/ai-stats/`) пока не переименованы под Burn — это запланировано к v1.0. На пользователя смотрит уже **Burn**: иконка в Dock, имя в Spotlight/Cmd+Tab, capsule в menu bar.
...
**Settings → Аккаунт** — свой friend_code, аватарка, шаринг статистики:
...
## Аккаунт, синхронизация и публичный лидерборд

Ручной PAT больше не обязателен. Settings → «Аккаунт» → **«Войти через GitHub»** проводит OAuth-вход: токен для коммитов/LOC выдаётся автоматически (вместо ручного `github_token`), и заодно создаётся аккаунт для синка. PAT через `config.json` остаётся как legacy-путь.
...
- **Публичный лидерборд.** Тогл «Показывать в публичном лидерборде» в Settings → «Аккаунт». Выключен по умолчанию; включив, светишь handle, аватар и цифры публично на сайте. Лидерборд друзей остаётся приватным, в приложении.
- **Приватность.** Без логина app работает полностью локально, на сервер не уходит ничего. Логин нужен только для синка и лидербордов.
- **Scope GitHub.** По умолчанию только публичные репы; приватные — отдельным тоглом при входе.
