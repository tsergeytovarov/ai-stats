...
final class DatabaseTests: XCTestCase {
    func test_migrate_creates_tables_and_indexes() throws {
        ...
        try dbq.read { db in
            ...
            XCTAssertEqual(Set(tables), ["ai_usage", "ai_usage_model", "analytics_ingest_state", "analytics_meta", "analytics_rate_limits", "analytics_turns", "friend_profiles", "github_activity", "github_loc_daily", "grdb_migrations", "leaderboard_cache", "limit_fetch_state", "limit_snapshots", "my_profile", "pending_snapshots", "sync_state"])
            ...
            XCTAssertTrue(indexes.contains("idx_ai_usage_model_day"))
            XCTAssertTrue(indexes.contains("idx_ai_usage_model_model"))
            XCTAssertTrue(indexes.contains("idx_github_day"))
            XCTAssertTrue(indexes.contains("idx_github_loc_daily_day"))
            XCTAssertTrue(indexes.contains("idx_analytics_turns_day"))
        }
    }
